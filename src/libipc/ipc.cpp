...
template <std::size_t DataSize, std::size_t AlignSize> struct msg_t;

template <std::size_t AlignSize> struct msg_t<0, AlignSize> {
  msg_id_t cc_id_;
  msg_id_t id_;
  std::int32_t remain_;
  bool storage_;
...
template <typename T> ipc::buff_t make_cache(T &data, std::size_t size) {
...
struct conn_info_head {
  ...
  conn_info_head(char const *prefix, char const *name)
      : prefix_{ipc::make_string(prefix)}, name_{ipc::make_string(name)},
        cc_id_{} {}
...
struct chunk_info_t {
  ...
  IPC_CONSTEXPR_ static std::size_t
  chunks_mem_size(std::size_t chunk_size) noexcept {
...
auto &chunk_storages() {
  class chunk_handle_t {
    ...
    static bool make_handle(ipc::shm::handle &h, ipc::string const &shm_name,
      ...
      if (!h.valid() &&
        ...
        ipc::error("[chunk_storages] chunk_shm.id_info_.acquire failed: "
                   "chunk_size = %zd\n",
                   chunk_size);
        return false;
      }
  ...
  public:
    ipc::shm::handle *get_handle(std::size_t chunk_size) {
      ipc::string pref("__ipc__");
      ipc::string shm_name{
    ...
    chunk_info_t *get_info(conn_info_head *inf, std::size_t chunk_size) {
      auto h = get_handle(chunk_size);
      auto *info = static_cast<chunk_info_t *>(h->get());
      if (info == nullptr) {
        ipc::error("[chunk_storages] chunk_shm.id_info_.get failed: chunk_size "
                   "= %zd\n",
                   chunk_size);
        return nullptr;
      }
  ...
  static ipc::map<std::size_t, chunk_handle_ptr_t> chunk_hs = [] {
    static constexpr std::size_t chunk_sizes[] = {
        64, 128, 256, 512, 5120, 1024, 2048, 4096, 8192, 16384, 32768};

    ipc::map<std::size_t, chunk_handle_ptr_t> chunks;
    for (auto chunk_size : chunk_sizes) {
      auto res = chunks.emplace(
          chunk_size,
          chunk_handle_ptr_t{ipc::mem::alloc<chunk_handle_t>(),
                             [](chunk_handle_t *p) { ipc::mem::destruct(p); }});
      auto &chunk_handle = *res.first->second;
      if (!chunk_handle.get_handle(chunk_size)) {
        ipc::error("[chunk_storages] chunk_handle.get_handle failed: "
                   "chunk_size = %zd\n",
                   chunk_size);
      }
    }
...
chunk_info_t *chunk_storage_info(conn_info_head *inf, std::size_t chunk_size) {
  ...
  std::decay_t<decltype(storages)>::iterator it;
  {
    ...
    for (auto jt = storages.begin(); jt != storages.end(); ++jt) {
      if (jt->first >= chunk_size) {
        it = storages.emplace(chunk_size, std::move(jt->second)).first;
        chunk_size = jt->first;
        break;
      }
    }
    ...
    if (it == storages.end()) {
      ipc::error("[chunk_storage_info] chunk size too large: %zd\n",
                 chunk_size);
      return nullptr;
    }
  }

  ipc::error("[chunk_storage_info] chunk size: %zd\n", chunk_size);
  return it->second->get_info(inf, chunk_size);
}

std::pair<ipc::storage_id_t, void *>
acquire_storage(conn_info_head *inf, std::size_t size, ipc::circ::cc_t conns) {
...
template <typename MsgT> bool clear_message(conn_info_head *inf, void *p) {
  ...
  struct conn_info_t : conn_info_head {
    ...
    static void clear_storage(char const *prefix, char const *name) noexcept {
      queue_t::clear_storage(
          ipc::make_prefix(ipc::make_string(prefix),
...
template <typename Policy> struct detail_impl {
  ...
  static bool reconnect(ipc::handle_t *ph, bool start_to_recv) {
    assert(ph != nullptr);
    assert(*ph != nullptr);
    chunk_storages(); // ensure chunk storage is initialized
    ...
    for (;;) {
      ...
      if (msg.storage_) {
        ...
        if (buf != nullptr) {
          ...
          ipc::log("fail: shm::handle for large message. msg_id: %zd, buf_id: "
                   "%zd, size: %zd\n",
  ...
  static ipc::buff_t try_recv(ipc::handle_t h) { return recv(h, 0); }
...
template <typename Flag> ipc::handle_t chan_impl<Flag>::init_first() {
...
template <typename Flag> void chan_impl<Flag>::disconnect(ipc::handle_t h) {
  detail_impl<policy_t<Flag>>::disconnect(h);
}

template <typename Flag> void chan_impl<Flag>::destroy(ipc::handle_t h) {
  disconnect(h);
...
template <typename Flag> char const *chan_impl<Flag>::name(ipc::handle_t h) {
  auto *info = detail_impl<policy_t<Flag>>::info_of(h);
  return (info == nullptr) ? nullptr : info->name_.c_str();
}

template <typename Flag> void chan_impl<Flag>::clear(ipc::handle_t h) noexcept {
  disconnect(h);
...
template <typename Flag> buff_t chan_impl<Flag>::try_recv(ipc::handle_t h) {
